...
class TIDERun:
	...
	def __init__(self, gt:Data, preds:Data, pos_thresh:float, bg_thresh:float, mode:str, max_dets:int, run_errors:bool=True, task:str=None):
		self.gt     = gt
		self.preds  = preds
	...
	def _run(self):
		...
		self.ap = self.ap_data.get_mAP(self.task)
	...
	def fix_main_errors(self, progressive:bool=False, error_types:list=None, qual:Qualifier=None) -> dict:
		...
		for error in error_types:
			...
			new_ap = _ap_data.get_mAP(self.task)
	...
	def fix_special_errors(self, qual=None) -> dict:
		return {
			FalsePositiveError: self.fix_errors(transform=FalsePositiveError.fix).get_mAP(self.task)    - self.ap,
			FalseNegativeError: self.fix_errors(false_neg_dict=self.false_negatives).get_mAP(self.task) - self.ap}
...
class TIDE:
   ...
   """
	...
	def evaluate(self, gt:Data, preds:Data, pos_threshold:float=None, background_threshold:float=None,
					   mode:str=None, name:str=None, use_for_errors:bool=True, task:str=None) -> TIDERun:
		...
		run = TIDERun(gt, preds, pos_thresh, bg_thresh, mode, gt.max_dets, use_for_errors, task)
	...
	def evaluate_range(self, gt:Data, preds:Data, thresholds:list=COCO_THRESHOLDS, pos_threshold:float=None,
							background_threshold:float=None, mode:str=None, name:str=None, task:str=None) -> dict:
		...
		for thresh in thresholds:
			...
			run = self.evaluate(gt, preds, pos_threshold=thresh, background_threshold=background_threshold,
				mode=mode, name=name, use_for_errors=(pos_threshold == thresh), task=task)
