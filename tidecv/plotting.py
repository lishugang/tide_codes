...
class Plotter():
	...
	def make_summary_plot(self, out_dir:str, errors:dict, model_name:str, rec_type:str, hbar_names:bool=False):
		...
		hor_bar = sns.barplot(data=error_dfs['main'], x='Delta mAP', y='Error Type', ax=ax,
					palette=self.colors_main.values())
		hor_bar.bar_label(ax.containers[0], fmt='%.2f', fontsize = 18)
		...
		ver_bar = sns.barplot(data=error_dfs['special'], x='Error Type', y='Delta mAP', ax=ax,
					palette=self.colors_special.values())
		ver_bar.bar_label(ax.containers[0], fmt='%.2f', fontsize = 18)
